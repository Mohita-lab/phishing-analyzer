...
class SimplePhishingAnalyzer:
    ...
    def analyze(self, email_text: str, attachments: List[Dict] = None) -> Dict:
        ...
        Raises:
            ValueError: if email_text is None or empty
        """
        # FIX 1: Guard against None / empty input
        if not email_text or not email_text.strip():
            raise ValueError("email_text is required and must not be empty.")
        ...
        # FIX 2: Clamp risk score so it never goes negative in the result
        final_score = max(0, self.risk_score)

        return {
            'risk_score':  final_score,
            'risk_level':  self._get_risk_level(final_score),
            'is_phishing': final_score >= 40,
    ...
    def _check_attachments(self, attachments: List[Dict]) -> Dict:
        ...
        for attachment in attachments:
            ...
            for double_ext in dangerous_double_extensions:
                if name.endswith(double_ext):
                    ...
                    self._add_indicator(35, 'Double Extension Attachment',
                        f'Attachment "{attachment.get("name")}" uses double extension: {double_ext}')
            ...
            if any(x in name for x in ['password', 'protected', 'encrypted', 'secret']):
                attachment_result['is_suspicious'] = True
                attachment_result['reasons'].append('Filename suggests password protection')
                self._add_indicator(20, 'Password-Protected Archive',
                    f'Attachment "{attachment.get("name")}" appears to be password-protected.')
    ...
    # FIX 2b: Accept score parameter so clamped value is used for level
    def _get_risk_level(self, score: int = None) -> str:
        s = score if score is not None else self.risk_score
        if s >= 50: return 'HIGH'
